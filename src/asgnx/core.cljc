...
(defn ask-experts [experts {:keys [args user-id]}]
  ...
  (cond
    (nil? experts) [[] "There are no TA on that class."]
    :else [[] (str "Succesfully register for an office hour slot. There are " (query-info user-id) " students before you.")]))
...
(defn answer-question [conversation {:keys [args user-id]}]
  (if (= 0 (count Queue))
    [[] "The queue is empty."]
    [[(action-send-msg (get Queue 0) "It's your turn")] (helper)]))

(defn helper [] (def Queue (into [] (rest Queue))) "Your message was sent.")
